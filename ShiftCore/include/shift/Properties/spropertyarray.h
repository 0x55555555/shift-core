...
public:
  ...
  Property *add(const PropertyInformation *info, const PropertyNameArg &name, PropertyInstanceInformationInitialiser *init, xsize index=X_UINT8_SENTINEL)
    {
    return PropertyContainer::addProperty(info, index, name, init);
    }

  Property *add(const PropertyInformation *info, xsize index=X_UINT8_SENTINEL, const PropertyNameArg &name=PropertyNameArg())
    {
