...
void SProcessManager::preCompute(const PropertyInstanceInformation *info, PropertyContainer *ent)
  {
  return;
  xAssert(g_manager);
    ...
    // lock scope
      ...
      while(success && g_manager->spareWorkers.size())
        ...
        while(i<numJobs && worker->job() == 0)
          ...
          if(!jobs[i]->_flags.hasFlag(Property::PreGetting) &&
              jobs[i]->_flags.hasFlag(Property::Dirty))
            {
            xAssert(jobs[i]);

            jobs[i]->_flags.setFlag(Property::PreGetting);
      ...
      xAssert(mainJob);
      if(!mainJob->_flags.hasFlag(Property::PreGetting) &&
         mainJob->_flags.hasFlag(Property::Dirty))
        {
        mainJob->_flags.setFlag(Property::PreGetting);
        }
      else
        {
        mainJob = 0;
  ...
  while(neededToWait)
    ...
    for(xsize i=0; i<numJobs; ++i)
      {
      if(jobs[i]->_flags.hasFlag(Property::Dirty) ||
         jobs[i]->_flags.hasFlag(Property::PreGetting))
        {
        l.unlock();
